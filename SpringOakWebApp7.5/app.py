...
from sqlalchemy import String, asc, func  # This is the correct import for datetime.strptime
...
def resident_database():
    if request.method == 'POST':
        ...
        new_resident = Resident(
            ...
            emergency_contact_full_name=request.form.get('emergency_contact_full_name'),  # Optional field, defaults to None
            emergency_contact_phone_number=request.form.get('emergency_contact_phone_number')  # Optional field, defaults to None
        )
...
def edit_resident(resident_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('resident_profile', room_number=resident.room_number, resident_last_name=resident.last_name))
...
def register():
    if request.method == 'POST':
        ...
        resident = Resident.query.filter_by(last_name=resident_last_name, room_number=resident_room_number).first()
        
        if resident is None:
            flash('Resident not found. Please check the details and try again.', 'error')
...
def visitor_dashboard():
    if request.method == 'POST':
        ...
        local_time = utc_time.replace(tzinfo=ZoneInfo("UTC")).astimezone(eastern_timezone)
...
def delete_visitor(visitor_id):
    ...
    return redirect(url_for('visitors'))  # Assuming 'visitors' is the endpoint for the visitor list
